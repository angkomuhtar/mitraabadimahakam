...
async function UPDATE_DATA_MASTER (data) {
    ...
    try {
        ...
        const tw_gross = await DailyRitaseCoalDetail.query(trx).where('ritasecoal_id', data.ritasecoal_id).getSum('w_gross')
        const tw_tare = await DailyRitaseCoalDetail.query(trx).where('ritasecoal_id', data.ritasecoal_id).getSum('w_tare')
        const tw_netto = await DailyRitaseCoalDetail.query(trx).where('ritasecoal_id', data.ritasecoal_id).getSum('w_netto')
