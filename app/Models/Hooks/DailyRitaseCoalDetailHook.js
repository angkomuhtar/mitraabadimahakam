...
async function UPDATE_DATA_MASTER(data) {
  ...
  /* GET PIT ID */
  const dailyRitaseCoal = (await DailyRitaseCoal.query(trx).with('daily_fleet').where('id', data.ritasecoal_id).last()).toJSON()

  // dailyfleet nya
  const { pit_id } = dailyRitaseCoal.daily_fleet

  const monthlyPlan = await MonthlyPlan.query(trx)
    .where(w => {
  ...
  try {
    if (data.checkout_jt) {
      console.log('RITASE ID >> ', data.ritasecoal_id)
      ...
      const coal_rit = await DailyRitaseCoalDetail.query(trx)
        .where(w => {
          w.where('w_netto', '>', 0)
      ...
      dailyPlan.merge({ actual: parseFloat(dailyPlan.actual) + data.w_netto })
