...
const createTime = (num) => {
	return `${String(num).padStart(2, '0')}:00:00`
}
class DailyRitaseController {
	...
	async storeUploadExcel({ request, auth }) {
		...
		//  steps
		// => Get data site, fleet, pit etc from dailyfleets use exca code
		// => Check if ritase already exist and get ritase id (if not create ritase)
		...
		const reqFile = request.file('detail-ritase-ob', validationOptions)
		...
		try {
			...
			const ExcelData = excelToJson({
				sourceFile: Helpers.publicPath(`/upload/dailyOB/`) + fileName,
				header: { rows: 2 },
				sheets: ['Data'],
				columnToKey: {
					A: 'exc_code',
			...
			for (const data of ObjectData) {
				...
				var hauler_id = _.findKey(unit, function (e) {
					return e === data.truck_code.replace(' ', '')
				})
				let dailyritase_id = ''
				// console.log(exca_id, hauler_id, data.exc_code, data.truck_code)

				// => Get data site, fleet, pit etc from dailyfleets use exca code
				const dataFleet = await db
					.select('mas_pits.site_id', 'fleet.pit_id', 'fleet.id as dailyfleet_id', 'equips.datetime')
					.from('daily_fleets as fleet')
					.innerJoin('daily_fleet_equips as equips', function () {
						this.on('fleet.id', 'equips.dailyfleet_id')
					})
					.innerJoin('mas_pits', function () {
						this.on('fleet.pit_id', 'mas_pits.id')
					})
					.where('date', date)
					.where('shift_id', data.shift == 'DS' ? 1 : 2)
					.where('equips.equip_id', exca_id)

				if (dataFleet.length != 1) {
					const err = new ValidationError()
					err.type = 'validate'
					throw new ValidationError(`daily fleets ${data.exc_code}, ${moment(date).format('DD MMMM YY')} \nnot founds `)
				}

				const { site_id, pit_id, dailyfleet_id } = dataFleet[0]

				const material = [
					{
						material: 'blasting',
						code: 15,
						data: data?.blasting || 0,
						volume: 22,
					},
					{
						material: 'ob',
				...
				// => Check if ritase already exist and get ritase id (if not create ritase)
				// const trx = await db.beginTransaction()
				// console.log('code Excavator ', data.exc_code, exca_id)
				for (const data_material of material) {
					if (data_material.data > 0) {
						const ritase = await DailyRitase.query()
							.where((w) => {
								w.where('date', date)
								w.where('site_id', site_id)
								w.where('pit_id', pit_id)
								w.where('dailyfleet_id', dailyfleet_id)
								w.where('exca_id', exca_id)
								w.where('shift_id', data.shift == 'DS' ? 1 : 2)
								w.where('material', data_material.code)
							})
							.last()
						if (ritase) {
							// => check ritase detail where jam and id_ritase
							dailyritase_id = ritase.id
						} else {
							// insert statement for ritase
							const ritase_i = await db
								.insert({
									dailyfleet_id,
									site_id,
									pit_id,
						...
						const cek_daily_ritase = await DailyRitaseDetail.query()
							.where((w) => {
								...
								w.where('dailyritase_id', dailyritase_id)
								w.where('check_in', `${date} ${createTime(data.start)}`)
							})
							.fetch()
						if (cek_daily_ritase.toJSON().length > 0) {
							throw new ValidationError(`data ritase ${data.exc_code}, ${data.truck_code}, ${moment(date).format('DD MMMM')} ${createTime(data.start)} - ${createTime(data.end)} already exists`)
						} else {
							// insert statement for details ritase
							let details = []
							for (let urut = 1; urut <= data_material.data; urut++) {
								details.push({
									dailyritase_id,
									checker_id,
									spv_id,
									hauler_id,
									opr_id: 26,
									check_in: `${date} ${createTime(data.start)}`,
									urut,
									duration: parseInt(60 / data_material.data),
									satuan: 'menit',
									material: data_material.code,
									volume: data_material.volume,
									status: 'Y',
								})
							}
			...
			return {
				success: false,
				message: err.type == 'Validate' ? err.message : 'Terjadi Kesalahan Sistem',
			}
		}
		...
		const isDailyPlan = await DailyPlan.query()
			.where((w) => {
		...
		if (BM.length > 0) {
			...
			for (const obj of GRP_BM) {
				...
				try {
					...
					console.log(
						'Total Blasting ::',
						obj.items.reduce((a, b) => {
							return a + b.material_blasting
						}, 0),
					)
					console.log(obj)
					for (const val of obj.items) {
