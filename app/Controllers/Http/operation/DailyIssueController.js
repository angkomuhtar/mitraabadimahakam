...
class DailyIssueController {
     ...
     async store({ auth, request }) {
          ...
          const req = request.except([
               'csrf_token',
               '_csrf',
               'submit',
          ])

          req.equip_id = _.isArray(req.equip_id)
               ? req.equip_id
               : [req.equip_id]
          ...
          for (const unit of req.equip_id) {
               ...
               if (req.event_id) {
                    try {
                         ...
                         await mamIssue.save()
                         await NotificationsHelpers.sendNotificationsIssue(
                              unit,
                              req.event_id,
                              req.description,
                              req.start_at,
     ...
     async show({ auth, params, view }) {
          ...
          console.log(data)
          return view.render('operation.daily-issue.show', {
               data: data,
          })
     }
     ...
     async update({ auth, params, request, view }) {
          ...
          let timesheet_id = timeSheet?.id || null
          const mamIssue = await MamIssue.query()
               .where('id', params.id)
               .last()
          if (req.event_id) {
               const dailyEvent = await DailyEvent.query()
                    .where('id', req.dailyevent_id)
                    .last()
               try {
                    dailyEvent.merge({
                         timesheet_id: timesheet_id,
     ...
     async destroy({ auth, params }) {
          ...
          await auth.getUser()
          try {
               const issue = await MamIssue.query()
                    .where('id', params.id)
                    .last()
               if (issue.dailyevent_id) {
                    await DailyEvent.query()
                         .where('id', issue.dailyevent_id)
                         .delete()
               }
