...
class DailyRitaseApiController {
     async GET_MONTH_EXCEL_DATA_PRODUCTION({
          ...
          return await DailyRitaseHelpers.GET_MONTH_EXCEL_DATA_PRODUCTION()
     }
     ...
     async filterByDate({ auth, request, response }) {
          var t0 = performance.now()
          const { begin_date, end_date, isFilter, filter } =
               request.only([
                    'begin_date',
                    'end_date',
                    'isFilter',
                              ...
                              dailyRitase = await DailyRitase.query()
                                   ...
                                   .where(wh => {
                                        ...
                                        if (_filter?.materials?.id) {
                                             wh.andWhere(
                                                  'material',
                                                  _filter.materials.id
                                             )
                                        }
                                   })
                                   .orderBy([
                                        {
                                             column: _filter?.order
                                                  ?.value,
                                             order: 'desc',
                                        },
                                   ])
     ...
     async create({ auth, request, response }) {
          ...
          async function SAVE_DATA() {
               ...
               try {
                    const checkDailyRitase = await DailyRitase.query(
                         trx
                    )
                         .where({
     ...
     async update({ auth, params, request, response }) {
          ...
          async function UPDATE_DATA() {
               const trx = await db.beginTransaction()
               try {
                    const daily_ritase = await DailyRitase.findOrFail(
                         id
                    )
