...
class Ritase {
      ...
      async GET_HOURLY_EXCEL_DATA(filePath, req, usr) {
            ...
            for (let shift of shifts) {
                  const startShift = moment(`${req.date} ${shift.start_shift}`).format(
                        'YYYY-MM-DD HH:mm:ss'
                  )
                  const endShift = moment(`${req.date} ${shift.start_shift}`)
                  ...
                  if (
                        ...
                        const __data__ = data.filter(
                              v => v.excaName && v.hauler && v.excaName !== 'COAL REHANDLE'
                        )
                        ...
                        for (const item of haulers) {
                              const excaNameFromExcel = (
                                    await MasEquipment.query().where('kode', item.excaName).first()
                              ).toJSON()
                                          ...
                                          for (const value of item.haulers) {
                                                const haulerId = (
                                                      await MasEquipment.query()
                                                            .where('kode', value.hauler)
                                                            .first()
                                                ).toJSON()

                                                for (
                                                      let index = 0;
                                                      index < value.totalMaterial;
                                                      index++
                                                ) {
                                                      var clock = moment(
                                                            `${req.date} ${
                                    ...
                                    if (data.data.length > 0 && data.dailyFleet.length > 0) {
                                          ...
                                          for (const exca of uniqExca) {
                                                ...
                                                for (let equipment of haulers) {
                                                      const equipData = await MasEquipment.query()
                                                            .where('kode', equipment)
                                                            .first()
                                                ...
                                                const excaID = await MasEquipment.query()
                                                      .where('kode', exca)
                                                      .first()
                                                ...
                                                if (blasting) {
                                                      ...
                                                      const materialName = await MasMaterial.query()
                                                            .where('name', 'Blasting')
                                                            .first()
                                                      dailyRitase.fill({
                                                            dailyfleet_id: df.id,
      ...
      async GET_HOURLY_EXCEL_DATA_v3(filePath, req, usr) {
            ...
            for (let shift of shifts) {
                  ...
                  if (
                        ...
                        const __data__ = data.filter(
                              v => v.excaName && v.excaName !== 'COAL REHANDLE'
                        )
                        ...
                        for (let x of haulers) {
                              const excaNameFromExcel = (
                                    await MasEquipment.query().where('kode', x.excaName).first()
                              ).toJSON()
      ...
      async GET_MONTH_EXCEL_DATA_PRODUCTION(filePath, req, usr) {
            ...
            for (const value of finalData) {
                  ...
                  for (const data of value.excaData) {
                        ...
                        let GET_EXCA_ID = await MasEquipment.query()
                        ...
                        const GET_PIT_ID = (await MasPit.query().where('name', PIT_NAME).first())
                              ?.id
                        const GET_SHIFT_ID = (
                              await MasShift.query().where('kode', data.shift).first()
                        )?.id

                        const GET_MATERIAL_DATA = await MasMaterial.query()
                        ...
                        dailyFleet.fill({
                              fleet_id:
                                    PIT_NAME === 'RPU' ? 23 : PIT_NAME === 'DERAWAN BARU' ? 21 : 22,
                              pit_id: GET_PIT_ID,
                        ...
                        if (dailyRitaseCheck) {
                              ...
                              await DailyRitaseDetail.query()
                                    .where('dailyritase_id', dailyRitaseCheck.id)
                                    .delete()
                        ...
                        for (const hauler of data.haulers) {
                              ...
                              let GET_HAULER_ID = await MasEquipment.query()
                         ...
                         \n`
                              ...
                              for (
                                    let totalRitase = 1;
                                    totalRitase <= parseInt(hauler.totalRitase);
                                    totalRitase++
                              ) {
                                    countRitase += 1
                              ...
                              `)
                                    ...
                                    dailyritaseDetail.fill({
                                          ...
                                          opr_id: null,
                                          check_in: `${data.date} ${await GET_SHIFT_DATA(
                                                data.shift
                                          )}`,
                                    })
                              ...
                              console.log(
                                    `---- Finished creating data for ${
                                          data.date
                                    } - ${EXCA_NAME} - ${
                                          data.shift
                                    } - ${HAULER_NAME} - ${PIT_NAME} - ${
                                          GET_MATERIAL_DATA.name
                                    } - ${countRitase} RIT  ${
            ...
            try {
                  ...
                  if (insertedDataLength !== originalDataLength) {
                        ...
                        for (const data of _temp) {
                              ...
                              for (const exca of data.excas) {
                                    excaData.push(
                                          GET_DATA_BY_EXCA_NAME(exca, data.date, data.shift)
                                    )
                              }
                              obj = {
                                    ...obj,
                        ...
                        for (const value of finalData) {
                              ...
                              for (const data of value.excaData) {
                                    ...
                                    const GET_PIT_ID = (
                                          await MasPit.query().where('name', PIT_NAME).first()
                                    )?.id
                                    const GET_SHIFT_ID = (
                                          await MasShift.query().where('kode', data.shift).first()
                                    )?.id
                                    ...
                                    for (const hauler of data.haulers) {
                                          ...
                                          console.log(
                                                `---- missing indexs finished ${
                                                      ...
                                                      GET_MATERIAL_DATA.vol *
                                                      parseInt(hauler.totalRitase)
                                                } BCM ---- `
                                          )
                                    }
                              }
