		...
		for (const value of finalData) {
			for (const data of value.pitData) {
				...
				for (const hauler of data.haulers) {
					...
					console.log(`---- finished creating data index : ${hauler.sheetIndex} daily fleet id : ${dailyFleet.id} ${data.date} - ${hauler.no_tiket} - ${data.shift} - ${HAULER_NAME} - ${PIT_NAME} - ${GET_SEAM_ID?.kode} - ${countRitase} RIT  ${hauler.netto} MT ---- `)
				}
			}
		}
				...
				for (const value of finalData) {
					for (const data of value.pitData) {
						...
						for (const hauler of data.haulers) {
							...
							console.log(`---- finished creating missing data index : ${hauler.sheetIndex} ${data.date} - ${hauler.no_tiket} - ${data.shift} - ${HAULER_NAME} - ${PIT_NAME} - ${GET_SEAM_ID?.kode} - ${countRitase} RIT  ${hauler.netto} MT ---- `)
						}
					}
				}
			}
