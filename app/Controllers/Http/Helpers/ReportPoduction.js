...
class repPoduction {
  ...
  async MW_WEEKLY(req) {
    ...
    for (let i = x - 1; i < y; i++) {
      ...
      var arrStart = moment(moment(req.week_begin).format('YYYY') + str)
        .startOf('week')
        .add(1, 'day')
      var arrEnd = moment(moment(req.week_end).format('YYYY') + str)
        .endOf('week')
        .add(1, 'day')
  ...
  async MW_HOURLY(req) {
    ...
    joinData = Object.keys(joinData).map(key => {
      return {
        ...
        sum_volume: joinData[key].reduce((a, b) => {
          return a + b.volume
        }, 0),
        items: joinData[key],
      }
  ...
  async PW_WEEKLY(req) {
    ...
    for (const obj of arrDate) {
      for (const val of arrPIT) {
        const sumACT = await DailyPlan.query()
          .where(w => {
            w.where('tipe', req.production_type)
            w.where('site_id', req.site_id)
  ...
  async PW_SHIFTLY(req) {
    ...
    let color = ['#7A92E5', '#C1CFFF', '#0334E2', '#02249E', '#E57590', '#FFC1D0', '#DF0136', '#A50329', '#EFD781', '#FFF0BB', '#EBB805', '#A98403']
    ...
    xresult = _.sortBy(xresult, function (num) {
      return num.stack
    })
    console.log(xresult)
