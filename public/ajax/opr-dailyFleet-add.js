$(function(){
    $('button#bt-open-equipment-list').on('click', function(e){
        e.preventDefault()
        $('div#tbl-equipment-list').show()
        $('div#tbl-equipment-select').hide()
    })

    $('button#bt-close-equipment-list').on('click', function(e){
        e.preventDefault()
    ...
    $('form#fm-dailyfleet').on('submit', function(e){
        e.preventDefault()
        var data = new FormData(this)
        $.ajax({
            async: true,
            url: '/operation/daily-fleet',
            method: 'POST',
            data: data,
            dataType: 'json',
            processData: false,
