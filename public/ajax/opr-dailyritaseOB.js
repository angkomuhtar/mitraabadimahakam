$(function () {
	...
	$('body').on('submit', 'form#fm-upload-ritase-ob', function (e) {
		...
		if ($('#typeInput').is(':checked')) {
			if ($('#detail-ritase-ob').val() == '') {
				swal('file tidak ada', 'pilih file terlebih dahulu')
				return
			}
			// swal(
			// 	{
			...
			$.ajax({
				...
				success: function (result) {
					...
					if (result.success) {
						...
						swal({ type: 'error', text: result.message, title: 'error' })
					}
				},
				error: function (err) {
					// console.log(err)
					...
					const { message } = err
					swal({ type: 'error', text: message, title: 'error' })
				},
				complete: () => {
					$('#loader').hide()
				},
			...
			swal('Feature Disabled', 'untuk sementara fitur input dengan metode form tidak dapat di gunakan', 'warning')
		}
		...
		// var isUploadFile = $('#typeInput').is(':checked')
	...
	$('body').on('change', 'input[name="detail-ritase-ob"]', function () {
		...
		data.append('detail-ritase-ob', $('#detail-ritase-ob')[0].files[0])
	...
	function addHauler() {
		$.ajax({
			...
			success: function (result) {
				...
				$('body')
					.find('tbody > tr.advance-table-row')
					.each(function (i, e) {
						console.log('test >> ', i)
						console.log($(this))
						$(this)
