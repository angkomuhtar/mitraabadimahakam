$(function () {
	...
	$('body').on('submit', 'form#filter-data', function (e) {
		...
		$.ajax({
			...
			contentType: false,
			beforeSend: function () {
				body.find('div#text-loading')
					.css('display', 'block')
					.html('<h3 class="text-center">Please Wait,</h3>' + '<small>Sedang melakukan collecting data...</small>')
			},
			success: function (result) {
				console.log(result)
				if (result.group === 'MW') {
			...
			error: function (err) {
				...
				body.find('div#text-loading')
					.css('display', 'block')
					.html('<h3 class="text-center">Error data,</h3>' + '<small>Chart tidak dapat di tampilkan...</small>')
			},
		})
	})
	...
	function showChart_MW(result) {
		...
		console.log(series)
		console.log('series', (xAxis, series, site, pit))
	...
	function BAR_CHART_PW(xAxis, series, site) {
		...
		Highcharts.chart('container', {
			...
			tooltip: {
				formatter: function () {
					return '<b>' + this.x + '</b><br/>' + this.series.name + ': ' + this.y + 'BCM<br/>' + 'PIT: ' + this.series.userOptions.stack
				},
			},
			plotOptions: {
				series: {
